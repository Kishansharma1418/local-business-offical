...
<body>
    ...
    <table>
        <tr>
            <th style="text-align:center;" colspan="4">
                Raw Materials issuance Sheet (Active Ingredient(s), excipients etc.)
            </th>
        ...
        <tbody>
            @foreach ($batch->items as $i => $item)
                ...
                <td>{{ $item->analytical_ref ?? '-' }}</td> -->
                    ...
                    <td><strong>{{ number_format($item->final_quantity, 3) }}
                            {{ strtoupper($item->uoms?->name) }}</strong>
                    </td>
                </tr>
            @endforeach
        </tbody>
    ...
    <table class="table table-bordered text-center mt-3">
        <tr>
            ...
            <!-- STORE -->
            <td width="33%">
                <strong class="d-block">Materials Issued By</strong>
