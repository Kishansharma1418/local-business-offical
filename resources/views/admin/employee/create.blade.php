...
@section('content')
    <div class="main-content-container overflow-hidden">
            ...
            <nav aria-label="breadcrumb">
                <ol class="breadcrumb align-items-center mb-0 lh-1">
                    ...
                    <li class="breadcrumb-item active">
                        <span class="text-secondary">Add Employee</span>
                    </li>
                </ol>
            </nav>
        </div>
        ...
        <form method="POST" action="{{ route('employee.store') }}" enctype="multipart/form-data" class="needs-validation"
            novalidate>
            @csrf
            <div class="row">
                <div class="col-lg-12">
                        ...
                        <div class="row">
                            ...
                            <div class="col-lg-4 mb-20">
                                ...
                                <input type="date" class="form-control" name="dob"
                                    ...
                                    value="{{ old('dob', now()->subYears(15)->format('Y-m-d')) }}" required>
                            </div>
...
<div class="col-lg-4 mb-20">
                    ...
                    <select name="reporting_id" class=" form-control">
                        <option value="">Select Employee</option>

                        @foreach ($employee as $emp)
                            <option value="{{ $emp->id }}"
                                {{ old('reporting_id') == $emp->id ? 'selected' : '' }}>
                                {{ $emp->full_name }}
                            </option>
                        @endforeach
                         ...
                         <div class="col-lg-4 mb-20">
                                ...
                                <input type="text" class="form-control" name="sales_head" value="{{ old('sales_head') }}"
                                    id="sales_head" required>
                            </div>
                                ...
                                <select name="designation_id" id="designation_id" class="form-control" required>
                                    ...
                                    @foreach ($designaions as $desinat)
                                        <option value="{{ $desinat->id }}"
                                            {{ old('designation_id') == $desinat->id ? 'selected' : '' }}>
                                            {{ $desinat->name }}
                                        </option>
                                    @endforeach
                                </select>
                            ...
                            <div class="col-lg-4 mb-20">
                                ...
                                <select class="form-control" name="separation_type">
                                    ...
                                    <option value="Absconding"
                                        {{ old('separation_type') == 'Absconding' ? 'selected' : '' }}>Absconding</option>
                                </select>
                            </div>

                            <div class="col-lg-4 mb-20">
                                <label class="label fs-16 mb-2">Separation Remark</label>
                                <textarea class="form-control" name="separation_remarks" placeholder="Separation Remark">{{ old('separation_remarks') }}</textarea>
                            </div>
                            <div class="col-lg-4 mb-20">
                                <label class="label fs-16 mb-2">Relieving Approved Date</label>
                                <input type="date" class="form-control" id="relieving_approvel_date"
                                    name="relieving_approvel_date" value="{{ old('relieving_approvel_date') }}">
                            </div>
                            ...
                            <div class="col-lg-4 mb-20">
                                ...
                                <input type="text" class="form-control" name="emergancy_contact_name"
                                    value="{{ old('emergancy_contact_name') }}">
                            </div>
                            <div class="col-lg-4 mb-20">
                                ...
                                <input type="text" class="form-control" name="emergancy_contact_number"
                                    value="{{ old('emergancy_contact_number') }}">
                            </div>
                            ...
                            <div class="col-lg-4 mb-20">
                                ...
                                <input type="text" class="form-control" name="aadhaar_no"
                                    value="{{ old('aadhaar_no') }}" maxlength="12" pattern="[0-9]{12}" required
                            ...
                            <div class="col-lg-4 mb-20">
                                ...
                                <input type="text" class="form-control" name="uan_no" value="{{ old('uan_no') }}">
                            </div>
                            ...
                            <div class="col-lg-4 mb-20">
                                ...
                                <select class="form-control" name="pf_aplicable" required>
                                    <option value="0" {{ old('pf_aplicable') == '0' ? 'selected' : '' }}>No</option>
                                    <option value="1" {{ old('pf_aplicable') == '1' ? 'selected' : '' }}>Yes</option>

                                </select>
                            </div>

                       <div class="col-lg-4 mb-20" id="pfNumberSection" style="display:none;">
    <label class="label fs-16 mb-2">
    ...
    @enderror
                            ...
                            <div class="col-lg-4 mb-20">
                                ...
                                <select class="form-control" name="esi_aplicable" required>
                                    <option value="0" {{ old('esi_aplicable') == '0' ? 'selected' : '' }}>No</option>
                                    <option value="1" {{ old('esi_aplicable') == '1' ? 'selected' : '' }}>Yes
                                    </option>

                                </select>
                            </div>
                          <div class="col-lg-4 mb-20" id="esiNumberSection" style="display:none;">
    <label class="label fs-16 mb-2">
        ESI Number <span class="text-danger">*</span>
    </label>
    <input type="text"
        class="form-control @error('esi_number') is-invalid @enderror"
        name="esi_number"
...
@push('scripts')
    ...
    <script>
        $(document).ready(function() {
            setupRealtimeValidation('Employee', 'code', '#code');
            setupRealtimeValidation('Employee', 'official_mail', 'input[name="official_mail"]', );
        });
    </script>
    <script>
        $(document).ready(function() {
...
$(document).ready(function(){
    ...
    <script>
        document.getElementById("joining_date").addEventListener("change", function() {
            let joinDate = this.value;
            document.getElementById("relieving_date").setAttribute("min", joinDate);
        });
    </script>
    <script>
        document.getElementById("relieving_date").addEventListener("change", function() {
            let joinDate = this.value;
            document.getElementById("relieving_approvel_date").setAttribute("min", joinDate);
        });
    ...
    document.querySelector('form').addEventListener('submit', function(e) {
        const email  = document.getElementById('official_mail').value.trim();
        const mobile = document.getElementById('mobile_no').value.trim();

        if (!email && !mobile) {
            e.preventDefault();
            document.getElementById('official_mail').classList.add('is-invalid');
            document.getElementById('mobile_no').classList.add('is-invalid');

            // Show error message
            let errDiv = document.getElementById('contact_error');
            if (!errDiv) {
                errDiv = document.createElement('div');
                errDiv.id = 'contact_error';
                errDiv.className = 'alert alert-danger mt-2';
                errDiv.innerText = '⚠️ Please enter either Official Email or Mobile No.';
                document.getElementById('mobile_no').closest('.col-lg-4').after(errDiv);
            }
    ...
    // Clear highlights as user types
    ['official_mail', 'mobile_no'].forEach(function(id) {
        document.getElementById(id).addEventListener('input', function() {
            document.getElementById('official_mail').classList.remove('is-invalid');
